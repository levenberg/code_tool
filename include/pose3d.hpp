...
Vec2i projection2d(Vec3 pt3d, Mat3 K, int height, int width)
{
    if(pt3d.z()<=0) return Vec2i(-1, -1);

    Vec3 pt_tmp= K*pt3d;
    int x= floor(pt_tmp.x()/pt_tmp.z());
    int y= floor(pt_tmp.y()/pt_tmp.z());

    if (x<0 || x>= width) return Vec2i(-1, -1);
    if (y<0 || y>= height) return Vec2i(-1, -1);
